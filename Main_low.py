...
class HyFA2U(nn.Module):#(B,3,8H,8W)
    def __init__(self):
        ...
        self.hyfeat = HyFeat()
        self.a2b=A2B()
        self.con=nn.Conv2d(125, 3, kernel_size=3, padding=1)
...
class Dynamic_weight(nn.Module):
    def __init__(self):
        ...
        self.con=nn.Conv2d(in_channels=64, out_channels=1, kernel_size=3, padding=1)
...
for epoch in range(num_epochs):
    ...
    print(f"Epoch [{epoch}/{num_epochs}] Batch [{i+1}/{len(data_loader)+1}] D Loss: {d_loss.item():.4f} G Loss: {g_loss.item():.4f}")